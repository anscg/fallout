...
| `app/controllers/admin/featured_projects_controller.rb` | Tabs: `active` / `archived`. Custom actions: `projects_search` (scoped_search autocomplete over `public_for_explore`, newest-first, excluding already-featured), `reorder` (bulk position update inside a transaction — explicit broadcast since `update_column` skips callbacks), `update_note`, `restore` (re-validates project is kept/listed before un-discarding, appends to end of position list). |
