...
- **Non-production writes fail closed**: `guard_external_writes!` runs at all three outbound HTTP sites (`batch_sync!`, `upload_or_create!`, `upload_attachment!`) and raises unless `Rails.env.production?` or `ALLOW_AIRTABLE_WRITES=1`. Airtable rows carry env-derived values (see the unified justification's inspect URL below), so a dev process pointed at the production database would otherwise overwrite live rows with dev-scoped data — this happened once and left 11 YSWS rows with unopenable `localhost:3000` inspect links. `no_upload` CSV dumps are unaffected; only the HTTP calls are gated.
...
2. `ShipUnifiedAirtableUploadJob` — fast path. Calls `Ship#upload_to_unified_airtable!`, which builds a fields hash (HCA identity name/birthday/primary address, repo/demo links, project description, ship id) and POSTs/PATCHes via `AirtableSync.upload_or_create!`. Identifier suffix `"Ship#<id>/unified"` keeps the local `AirtableSync` row distinct from the cron mirror's `"Ship#<id>"`. `upload_or_create!` persists the returned airtable_id back so retries PATCH the existing row instead of creating duplicates (a hackworks gotcha we deliberately don't replicate). No Screenshot field is included here — that's handled by the second job via a different endpoint.
   ...
   The `"Optional - Override Hours Spent Justification"` field is rendered by `JustificationRenderer`, whose `INSPECT_URL` token expands to `UnifiedInspectToken.url_for(ship.id)` — an absolute, non-expiring presigned link to `/admin/unified_inspect/:ship_id/:token` that lets YSWS auditors open the inspector without a Fallout account. Both halves are env-derived: the host comes from `APP_HOST` and the token is HMAC-SHA256 keyed by `EXTERNAL_API_KEY`. `url_for` raises outside development when `APP_HOST` is unset rather than emitting a `localhost` URL, and rotating `EXTERNAL_API_KEY` invalidates every link already published to Airtable (they 404 — the route's `valid?` check fails). Re-signing after a rotation means PATCHing the justification field on every affected YSWS row.

3. `AttachShipUnifiedScreenshotJob` — slow path. Finds a source URL via `ShipChecks::UnifiedScreenshotFinder` (four-stage strategy below), caches it on `ship.frozen_screenshot`, processes via `ShipChecks::UnifiedScreenshotProcessor` (libvips → JPEG, progressive quality reduction until ≤5MB; supports PNG/JPG/WEBP/GIF plus PDF), then POSTs the bytes to `https://content.airtable.com/v0/{base}/{recordId}/Screenshot/uploadAttachment` via `AirtableSync.upload_attachment!`. The job retries with `wait: 15.seconds, attempts: 8` if the parallel upload job hasn't yet created the Airtable record (no airtable_id in `AirtableSync`). After a successful attachment, writes a sentinel `AirtableSync` row keyed `"Ship#<id>/unified/screenshot"` so retries skip — `uploadAttachment` _appends_ to the field array, so a repeat would duplicate the screenshot. PDF sources go through `ShipChecks::PdfRasterizer` first, which renders page 1 to PNG using `pdftoppm` in a child process with an empty environment, rlimits, and a wall-clock kill. libvips itself never opens a PDF: Rails 8.1.3.1 calls `Vips.block_untrusted(true)` at boot for CVE-2026-66066, disabling `pdfload`/`svgload` process-wide, and we do not re-enable either. `poppler-utils` is installed in `Dockerfile.worker` only — all three callers are background jobs. No separate font package is needed; base-14 fonts resolve through poppler's own dependencies, verified by rendering text in a bookworm container with no font package installed. If `pdftoppm` is absent the rasterizer logs and returns nil, so the thumbnail is skipped rather than the job failing. SVG sources are still skipped; there is no out-of-process path for them.
