...
class KoiTransaction < ApplicationRecord
  # Reasons that require a ship_id (DB-enforced via partial unique indexes too)
  SHIP_REASONS = %w[ship_review built_irl_conversion].freeze
  REASONS = (SHIP_REASONS + %w[admin_adjustment streak_goal]).freeze
  ...
  # Belt-and-suspenders alongside the partial unique indexes; this validation
  # guards against malformed inserts (wrong reason, stray ship_id).
